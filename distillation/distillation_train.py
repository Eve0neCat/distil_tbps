...
import os
import sys
sys.path.append("/path/to/distil_tbps/")
...
if __name__ == '__main__':
    ...
    # 在 distillation_train.py 中添加
    logger.info(f"use_task_loss: {student_config.distillation_config.get('use_task_loss', False)}")
    logger.info(f"task_loss types: {student_config.distillation_config.get('task_loss', 'None')}")
    ...
    #     evaluator = Evaluator(val_img_loader, val_txt_loader)
    # else:
    #     evaluator = Evaluator2(val_img_loader, val_txt_loader) 
    if args.dataset_name == "CUHK-PEDES" or args.dataset_name == "RSTPReid":        
